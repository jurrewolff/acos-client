...
class BaseL7(base.BaseV30):
    ...
    def _set(self, name, insert_client_ip=0, insert_client_ip_header_name="", client_ip_hdr_replace=0, 
        ...
        # Everything except insert_client_ip and request header insert.
    ...
    def create(self, name, insert_client_ip=False, insert_client_ip_header_name="", client_ip_hdr_replace=0, request_header_insert_list=[], response_header_insert_list=[], **kwargs):  # Custom - cipher_cfg
        if self.exists(name):
            raise acos_errors.Exists

        self._set(name, insert_client_ip, insert_client_ip_header_name, client_ip_hdr_replace, request_header_insert_list, response_header_insert_list, **kwargs)  # Custom - cipher_cfg

    def update(self, name, insert_client_ip=False, insert_client_ip_header_name="", client_ip_hdr_replace=0, request_header_insert_list=[], response_header_insert_list=[], **kwargs):
        self._set(name, insert_client_ip, insert_client_ip_header_name, client_ip_hdr_replace, request_header_insert_list, response_header_insert_list, update=True, **kwargs)
